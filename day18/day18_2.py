...
numbers = set()
while line := input():
    numbers.add(read_snail_number(line))
...
for number_a in numbers:
    for number_b in numbers:
        if number_a == number_b:
            continue
        sum_numbers = snail_add(copy.deepcopy(number_a), copy.deepcopy(number_b))
        magnitude = sum_numbers.magnitude()
        print(f'{number_a} + {number_b} ({magnitude})')
